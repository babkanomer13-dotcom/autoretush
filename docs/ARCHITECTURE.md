...
5. one-to-one назначение пар внутри одной рабочей папки;
6. строгие совпадения принимаются, пограничные показываются человеку, остальные исключаются;
7. непосредственно перед materialization каждая выбранная пара повторно выравнивается в полном разрешении и обязана пройти явные пороги edge-correlation и overlap.

Последний барьер работает до split и копирования. Пара без geometry, с нечитаемым
кадром, невалидной homography или проваленным QC в датасет не попадает; наружу выводятся
только агрегированные причины без исходных путей. Полноразмерные выровненные дубликаты
не сохраняются: локальный загрузчик применяет homography и строит `valid_mask` на лету.
